...
symbol_node_t *create_symbol_node(const char* symbol, const char *type, const int scope_num, const int line_number){
...
void symbol_table_insert(symbol_node_t **symbol_table, const char* symbol, const int scope_num, const char *type, const int line_number){
   ...
   symbol_node_t *node = create_symbol_node(symbol, type, scope_num, line_number);
...
void symbol_table_print(symbol_node_t **symbol_table, const char *table_name){
   ...
   printf(FORE_GRN "|\t%-20.20s" "|\t%-20.20s" "|\t%-20.20s|" "|\t%-20.20s|" "\n" RESET, "Symbol", "Scope_Num", "Type", "Line Number");
   printf(FORE_YEL DASHES RESET);

   for(int i=0; i< MAX_SYMBOL_TABLE_SIZE; ++i){
      for(symbol_node_t *curr = symbol_table[i]; curr != NULL; curr = curr->next){
         printf(FORE_GRN "|\t%-20.20s" "|\t%-20d" "|\t%-20.20s" ATTR_BOLD "|\t%-20d|" "\n" RESET, curr->symbol, curr->scope_num, curr->type, curr->line_number);
      }
   }
